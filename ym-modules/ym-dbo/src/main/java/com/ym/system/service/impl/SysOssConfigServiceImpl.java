...
import com.ym.system.domain.bo.SysOssConfigBo;
import com.ym.system.domain.vo.SysOssConfigVo;
import com.ym.system.event.OssConfigChangeEvent;
import com.ym.system.mapper.SysOssConfigMapper;
import com.ym.system.service.ISysOssConfigService;
...
public class SysOssConfigServiceImpl implements ISysOssConfigService {
    ...
    public int updateOssConfigStatus(SysOssConfigBo bo) {
        ...
        SysOssConfig persisted = ossConfigMapper.selectById(sysOssConfig.getOssConfigId());
        if (persisted == null) {
        ...
        update.setOssConfigId(persisted.getOssConfigId());
        update.setStatus(sysOssConfig.getStatus());
        row += ossConfigMapper.updateById(update);
        if (row > 0) {
            persisted.setStatus(sysOssConfig.getStatus());
            SpringUtils.context().publishEvent(OssConfigChangeEvent.useDefault(
                persisted.getConfigKey(), JsonUtils.toJsonString(persisted)));
        }
        return row;
    }
