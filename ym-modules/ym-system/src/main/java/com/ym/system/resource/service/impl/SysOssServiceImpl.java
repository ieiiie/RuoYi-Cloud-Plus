...
import com.ym.common.oss.enums.AccessPolicy;
import com.ym.common.oss.factory.OssFactory;
import com.ym.common.oss.model.Options;
import com.ym.common.oss.model.PutObjectResult;
import com.ym.common.tenant.helper.TenantHelper;
import com.ym.system.resource.domain.SysOss;
...
import com.ym.system.resource.mapper.SysOssMapper;
...
public class SysOssServiceImpl implements ISysOssService {
    ...
    public SysOssVo upload(MultipartFile file, SysOssExt ossExt) {
        ...
        OssClient instance = currentTenantOssClient();
    ...
    /**
     * 获取当前租户绑定的 OSS 客户端，不使用全局默认配置。
     */
    private OssClient currentTenantOssClient() {
        String tenantId = TenantHelper.getTenantId();
        if (StringUtils.isBlank(tenantId)) {
            throw new ServiceException("当前请求缺少租户上下文");
        }
        Long configId = remoteTenantService.getOssConfigId(tenantId);
        SysOssConfig config = ossConfigMapper.selectById(configId);
        if (config == null || StringUtils.isBlank(config.getConfigKey())) {
            throw new ServiceException("租户绑定的OSS配置不存在");
        }
        return OssFactory.instance(config.getConfigKey());
    }
