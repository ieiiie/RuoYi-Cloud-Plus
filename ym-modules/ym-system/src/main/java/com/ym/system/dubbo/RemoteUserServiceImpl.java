...
import com.ym.common.core.enums.UserStatus;
...
import com.ym.common.core.utils.ThreadUtils;
...
import com.ym.system.api.model.PostDTO;
...
import com.ym.system.mapper.SysGlobalUserMapper;
...
import com.ym.system.service.ISysPermissionService;
...
import com.ym.system.service.PlatformTenantAccessService;
import com.ym.system.service.SysLoginPermissionService;
...
public class RemoteUserServiceImpl implements RemoteUserService {
    ...
    public LoginUser getUserInfoByGlobalUserId(Long globalUserId, String tenantId) throws UserException {
        ...
        if (SystemConstants.SUPER_ADMIN_GLOBAL_USER_ID.equals(globalUserId)) {
            return buildLoginUser(globalUser, platformTenantAccessService.enterTenant(globalUser, tenantId));
        }
    ...
    public List<RemoteTenantUserVo> listTenantUsers(Long globalUserId) {
        ...
        SysGlobalUser globalUser = requireActiveGlobalUser(globalUserId);
        if (SystemConstants.SUPER_ADMIN_GLOBAL_USER_ID.equals(globalUserId)) {
            return platformTenantAccessService.listTenants(globalUser);
        }
        List<SysUser> tenantUsers = TenantHelper.ignore(() -> userMapper.lambda()
    ...
    private LoginUser buildLoginUser(SysGlobalUser globalUser, SysUser tenantUser) {
        return TenantHelper.dynamic(tenantUser.getTenantId(), () -> {
            ...
            loginUser.setTenantLogoUrl(tenant == null ? null : tenant.getLogoUrl());
            ...
            if (SystemConstants.SUPER_ADMIN_GLOBAL_USER_ID.equals(globalUser.getGlobalUserId())) {
                SysLoginPermissionService.grantPlatformPermissions(loginUser);
            }
            return loginUser;
        });
    }
    ...
    public boolean updateUserProfile(RemoteUserProfileUpdateBo command) {
        ...
        return DataPermissionHelper.ignore(() -> {
            SysUser existing = userMapper.selectById(command.getUserId());
            if (existing == null) {
                return false;
            }
            int rows = userMapper.updateById(user);
            if (existing.getGlobalUserId() != null) {
                SysGlobalUser global = new SysGlobalUser();
                global.setGlobalUserId(existing.getGlobalUserId());
                global.setNickName(command.getNickName());
                global.setPhoneNumber(command.getPhoneNumber());
                global.setGender(command.getGender());
                globalUserMapper.updateById(global);
            }
            return rows > 0;
        });
    }
