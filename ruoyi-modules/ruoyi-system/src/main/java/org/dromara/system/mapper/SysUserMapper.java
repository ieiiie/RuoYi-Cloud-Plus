...
public interface SysUserMapper extends BaseMapperPlus<SysUser, SysUserVo>, MPJBaseMapper<SysUser> {
    ...
    default Page<SysUserVo> selectAllocatedList(Page<SysUserVo> page, SysUserBo user) {
        MPJLambdaWrapper<SysUser> wrapper = buildUserRoleJoinBuilder(user)
     ...
     * @return 用户角色关联查询包装器
     */
    default LambdaJoinQueryBuilder<SysUser> buildUserRoleJoinBuilder(SysUserBo user) {
        return buildUserGlobalJoinBuilder()
            .distinct()
            .leftJoin(SysUserRole.class, "sur", SysUserRole::getUserId, "u", SysUser::getUserId)
            .leftJoin(SysRole.class, "r", SysRole::getRoleId, "sur", SysUserRole::getRoleId)
            .likeIfText("gu", SysGlobalUser::getUserName, user.getUserName())
            .eqIfText("u", SysUser::getStatus, user.getStatus())
            .likeIfText("gu", SysGlobalUser::getPhoneNumber, user.getPhoneNumber());
    }
