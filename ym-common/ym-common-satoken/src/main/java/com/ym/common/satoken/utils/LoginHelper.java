package com.ym.common.satoken.utils;
...
public class LoginHelper {
    ...
    /**
     * 替换当前 token 会话中的有效租户用户。
     *
     * <p>切换租户不换发 token，同时将 Account-Session 终端归属迁移到目标租户
     * 本地用户。授权、菜单、数据权限等实时身份信息仍从 token session 内的
     * {@link LoginUser} 读取，并保留客户端和终端信息。</p>
     *
     * @param loginUser 切换后的租户成员登录信息
     */
    public static void updateLoginUser(LoginUser loginUser) {
        withTokenSessionLock(() -> {
            SaSession tokenSession = StpUtil.getTokenSession();
            LoginUser previous = getLoginUser(tokenSession);
            if (ObjectUtil.isNotNull(previous)) {
                preserveSessionContext(previous, loginUser);
            }
            rebindAccountSession(loginUser);
    ...
    /**
     * 在与租户切换相同的锁内重读并刷新权限，不迁移账号身份或终端归属。
     * 回调只能更新当前成员的授权快照；异常时不持久化。
     */
    public static <T> T refreshLoginUser(Function<LoginUser, T> refresh) {
        return withTokenSessionLock(() -> {
            SaSession session = StpUtil.getTokenSession();
            LoginUser user = getLoginUser(session);
            if (user == null) {
                throw new ServiceException("当前登录会话不存在，请重新登录");
            }
            String loginId = user.getLoginId();
            String tenantId = user.getTenantId();
            Long globalUserId = user.getGlobalUserId();
            T result = refresh.apply(user);
            if (!Objects.equals(loginId, user.getLoginId())
                || !Objects.equals(tenantId, user.getTenantId())
                || !Objects.equals(globalUserId, user.getGlobalUserId())) {
                throw new ServiceException("刷新权限不能修改登录身份");
            }
    ...
    /**
     * 全局命名空间内按 token 串行化会话读改写；跨租户、跨服务使用同一把锁。
     * 使用看门狗续期，不改变 token 或会话 TTL。拿锁后重新检查登录状态。
     */
    public static <T> T withTokenSessionLock(Supplier<T> action) {
        StpUtil.checkLogin();
        String key = GlobalConstants.GLOBAL_REDIS_KEY + "satoken:session-lock:"
            + StpUtil.getLoginType() + ":" + DigestUtil.sha256Hex(StpUtil.getTokenValue());
        RLock lock = RedisUtils.getClient().getLock(key);
        try {
            if (!lock.tryLock(10, TimeUnit.SECONDS)) {
                throw new ServiceException("当前会话正在更新，请稍后重试");
            }
            try {
                StpUtil.checkLogin();
                return action.get();
            ...
            throw new ServiceException("当前会话更新被中断，请重试");
        }
    }

    /**
     * 将当前 token 的终端归属迁移到切换后的租户本地用户。
     */
    private static void rebindAccountSession(LoginUser loginUser) {
        String tokenValue = StpUtil.getTokenValue();
        Object previousLoginId = StpUtil.getLoginId();
        String targetLoginId = loginUser.getLoginId();
        if (StringUtils.equals(String.valueOf(previousLoginId), targetLoginId)) {
            return;
        }

        SaSession previousSession = StpUtil.stpLogic.getSessionByLoginId(previousLoginId, false);
        SaTerminalInfo previousTerminal = previousSession == null ? null : previousSession.getTerminal(tokenValue);
        long tokenTimeout = StpUtil.getTokenTimeout();
        SaSession targetSession = StpUtil.stpLogic.getSessionByLoginId(targetLoginId, true, tokenTimeout);
        SaTerminalInfo targetTerminal = new SaTerminalInfo()
            .setTokenValue(tokenValue)
            .setDeviceType(loginUser.getDeviceType())
            .setDeviceId(previousTerminal == null ? null : previousTerminal.getDeviceId())
            .setExtraData(previousTerminal == null ? null : previousTerminal.getExtraData())
            .setCreateTime(System.currentTimeMillis());

        targetSession.updateMinTimeout(tokenTimeout);
        targetSession.addTerminal(targetTerminal);
        try {
            StpUtil.stpLogic.updateTokenToIdMapping(tokenValue, targetLoginId);
        } catch (RuntimeException ex) {
            targetSession.removeTerminal(tokenValue);
            targetSession.logoutByTerminalCountToZero();
            throw ex;
        }
        if (previousSession != null) {
            previousSession.removeTerminal(tokenValue);
            previousSession.logoutByTerminalCountToZero();
        }
    }
    ...
    public static boolean isSuperAdmin() {
        return isPlatformSuperAdmin(currentIdentityOrNull());
    }

    /** 根据服务端会话中的全局身份判断；兼容尚无全局账号的独立 DBO 会话。 */
    public static boolean isPlatformSuperAdmin(LoginUser user) {
        if (user == null) {
            return false;
        }
        return user.getGlobalUserId() == null ? isSuperAdmin(user.getUserId())
            : SystemConstants.SUPER_ADMIN_GLOBAL_USER_ID.equals(user.getGlobalUserId());
    }

    /** 当前用户授权查询使用全局身份；查询其他用户时不继承调用方的管理员权限。 */
    public static boolean isCurrentSuperAdmin(Long userId) {
        LoginUser current = currentIdentityOrNull();
        return current != null && java.util.Objects.equals(current.getUserId(), userId)
            && isPlatformSuperAdmin(current);
    }

    private static LoginUser currentIdentityOrNull() {
        try {
            return getLoginUser();
        } catch (SaTokenContextException ignored) {
            // 登录构建的虚拟线程只有显式 tenantId，没有 Web/Dubbo token 上下文。
            // 不能继承调用线程的管理员特权，构建完成后再按全局账号授予。
            return null;
        }
    }
