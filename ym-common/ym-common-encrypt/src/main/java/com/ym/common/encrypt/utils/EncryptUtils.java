package com.ym.common.encrypt.utils;
...
public class EncryptUtils {
    ...
    /**
     * SM4 CBC/PKCS5Padding 加密，返回 Base64，用于对接要求固定 IV 的设备平台。
     */
    public static String encryptBySm4CbcBase64(String data, String password, String iv) {
        if (StrUtil.hasBlank(data, password, iv)) {
            throw new IllegalArgumentException("SM4 CBC 需要数据、密钥和 IV");
        }
        byte[] keyBytes = password.getBytes(StandardCharsets.UTF_8);
        byte[] ivBytes = iv.getBytes(StandardCharsets.UTF_8);
        if (keyBytes.length != 16 || ivBytes.length != 16) {
            throw new IllegalArgumentException("SM4 CBC 密钥和 IV 必须为 16 字节");
        }
