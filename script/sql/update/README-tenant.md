...
本次改造采用“共享库、共享表、`tenant_id` 行级隔离”模式，与 YM-Cloud-Plus 的核心模型一致。平台全局表（菜单、租户、套餐、客户端、OSS 配置及用户/角色关联表）不参与行级拦截；用户、角色、部门、字典、参数、日志、消息和 OSS 数据按租户隔离。
...
3. 已有数据库按数据库类型执行对应脚本：
   ...
   已完成 SaaS 运营迁移及后续 6.0.5 至 6.0.9 升级的 MySQL 数据库，继续执行：

   - MySQL：[update_6.0.10-tenant-concurrent-login.sql](update_6.0.10-tenant-concurrent-login.sql)

   6.0.10 新增租户可编辑的“用户并发登录数量”，并为全部存量租户补齐默认值
   `-1`（不限）。正整数按同一租户用户的 `device_type` 分别限制 PC、Android、
   iOS 和微信小程序会话。
