-- -----------------------------------------------------------------------------
-- RuoYi-Cloud-Plus 6.0.3 菜单清理脚本（MySQL）
--
-- 删除“系统工具”和“PLUS官网”菜单及其子菜单，并清理角色授权、租户套餐菜单引用。
...
DROP PROCEDURE IF EXISTS remove_update_6_0_3_system_tool_plus_menu;

DELIMITER //
CREATE PROCEDURE remove_update_6_0_3_system_tool_plus_menu()
BEGIN
    DECLARE removed_menu_ids TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT '';

    DROP TEMPORARY TABLE IF EXISTS tmp_update_6_0_3_removed_menu_ids;
    CREATE TEMPORARY TABLE tmp_update_6_0_3_removed_menu_ids (
        menu_id bigint(20) NOT NULL,
        PRIMARY KEY (menu_id)
    );

    INSERT IGNORE INTO tmp_update_6_0_3_removed_menu_ids (menu_id)
    WITH RECURSIVE menu_tree AS (
        SELECT menu_id
        FROM sys_menu
        WHERE menu_id IN (1761400000000000003, 1761400000000000004)
        UNION ALL
    ...
    FROM tmp_update_6_0_3_removed_menu_ids;

    DELETE role_menu
    FROM sys_role_menu role_menu
    INNER JOIN tmp_update_6_0_3_removed_menu_ids removed_menu
...
START TRANSACTION;
CALL remove_update_6_0_3_system_tool_plus_menu();
COMMIT;
DROP PROCEDURE remove_update_6_0_3_system_tool_plus_menu;
