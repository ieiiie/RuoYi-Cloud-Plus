...
-- Upstream: aizuda/snail-job tag vsj2.0.0, doc/sql/snail_job_mysql.sql.
-- Safety change: never drop an existing scheduler database from an initialization script.
CREATE DATABASE IF NOT EXISTS snail_job CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE snail_job;
...
(`namespace_id`, `group_name`, `description`, `token`, `group_status`, `version`, `group_partition`, `id_generator_mode`, `init_scene`)
VALUES
('764d604ec6fc45f68cd92514c40e9e1a', 'ym-agriculture', 'YM agriculture test executor',
 'SJ_YM_MIGRATION_TEST_ONLY_CHANGE_ME', 1, 1, 0, 1, 1),
('764d604ec6fc45f68cd92514c40e9e1a', 'ym-iot', 'YM IoT test executor',
 'SJ_YM_MIGRATION_TEST_ONLY_CHANGE_ME', 1, 1, 0, 1, 1)
ON DUPLICATE KEY UPDATE
`description` = VALUES(`description`), `group_status` = VALUES(`group_status`), `version` = `version` + 1;
